    ...
    describe("non-isolated mode", function () {
      it("should handle build infos and artifacts appropiately", async () => {
        ...
        for (const source of ["Foo.sol", "FooFactory.sol", "IFoo.sol"]) {
          assert.notEqual(
            secondSnapshot.artifacts[source][0].modificationTime,
            firstSnapshot.artifacts[source][0].modificationTime,
          );
        }
        ...
        for (const source of ["Foo.sol", "FooFactory.sol", "IFoo.sol"]) {
          assert.notEqual(
            secondSnapshot.typeFiles[source].modificationTime,
            firstSnapshot.typeFiles[source].modificationTime,
          );
        }
      });
    });
